<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","textAlign":"left","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">ID Number</label>
						<el-input v-model="searchForm.shenfenzheng" placeholder="ID Number" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">Approved</label>
						<el-select clearable v-model="searchForm.sfsh" placeholder="Approved">
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#333","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('bankakaihu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						Add
					</el-button>
					<el-button class="del" v-if="isAuth('bankakaihu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						Delete
					</el-button>


					<el-button class="btn18" v-if="isAuth('bankakaihu','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"border":"0px solid #eee","padding":"0","margin":"10px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)","width":"100%","fontSize":"12px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<!-- shenqingcailiao -->
					<!-- $column.hiden -->
					<el-table-column :resizable='true' :sortable='true' prop="shenqingcailiao" label="Application Materials">
						<template slot-scope="scope">
							<el-button v-if="scope.row.shenqingcailiao" type="text" size="small" @click="download(scope.row.shenqingcailiao)">Download</el-button>
					...
					<!-- shhf -->
					<!-- 是 -->
					<el-table-column :resizable='true' :sortable='true' prop="shhf" label="Review Reply" show-overflow-tooltip>
						<template slot-scope="scope">
					...
					<el-table-column :resizable='true' :sortable='true' prop="sfsh" label="Review Status">
						<template slot-scope="scope">
							<el-tag v-if="scope.row.sfsh=='否'" type="danger">Not Approved</el-tag>
							<el-tag v-if="scope.row.sfsh=='PendingReview'" type="warning">Pending Review</el-tag>
							<el-tag v-if="scope.row.sfsh=='是'" type="success">Approved</el-tag>
						</template>
					</el-table-column>
					
					<el-table-column width="300" label="Actions">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('bankakaihu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								Details
							</el-button>
							<el-button class="edit" v-if=" isAuth('bankakaihu','修改')  && scope.row.sfsh=='PendingReview' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('bankakaihu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<!-- Add/Modify Page  Pass the parent component's search method to the child component-->
		<add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>


		
		<el-dialog :title="this.batchIds.length>1?'Batch Review':'Review'" :visible.sync="sfshBatchVisiable" width="50%">
			<el-form ref="form" :model="form" label-width="80px">
				<el-form-item label="Review Status">
					<el-select v-model="shBatchForm.sfsh" placeholder="Review Status">
						<el-option label="Approved" value="是"></el-option>
						<el-option label="Not Approved" value="否"></el-option>
						<el-option label="Pending Review" value="PendingReview"></el-option>
					</el-select>
				</el-form-item>
			...
			<span slot="footer" class="dialog-footer">
				<el-button @click="sfshBatchVisiable=false">Cancel</el-button>
...
import AddOrUpdate from "./add-or-update";
    ...
    init () {
        this.sfshOptions = "yes,no,PendingReview".split(',');
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
    ...
    shBatchDialog(){
		for(let x in this.dataListSelections){
			if(this.dataListSelections[x].sfsh&&this.dataListSelections[x].sfsh!='PendingReview'){
				this.$message.error('There are already approved data, so batch approval is not possible');
				this.batchIds = []
				return false
			}
    ...
    //批量审核
    shBatchHandler(){
      this.$confirm(`是否${this.batchIds.length>1?'One-click review':'review'}selected data?`, "prompt", {
        confirmButtonText: "confirm",
        cancelButtonText: "cacnel",
        type: "warning"
      }).then(() => {
        this.$http({
          url: "bankakaihu/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
          method: "post",
          data: this.batchIds
        }).then(({ data }) => {
          if (data && data.code === 0) {
	...
	bankakaihustatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'bankakaihu/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
