			...
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","textAlign":"left","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">柜员工号</label>
						<el-input v-model="searchForm.juyuangonghao" placeholder="柜员工号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">柜员姓名</label>
						<el-input v-model="searchForm.juyuanxingming" placeholder="柜员姓名" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#333","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('juyuan','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('juyuan','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","padding":"0","margin":"10px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)","width":"100%","fontSize":"12px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="柜员手机">
						<template slot-scope="scope">
							{{scope.row.juyuanshouji}}
						</template>
					</el-table-column>
					<!-- touxiang -->
					<!-- $column.hiden -->
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='true' prop="touxiang" width="200" label="Photo">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="Option">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('juyuan','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								Details
							</el-button>
							<el-button class="edit" v-if=" isAuth('juyuan','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('juyuan','删除') " type="primary" @click="deleteHandler(scope.row.id )">
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.juyuangonghao!='' && this.searchForm.juyuangonghao!=undefined){
            params['juyuangonghao'] = '%' + this.searchForm.juyuangonghao + '%'
          }
           if(this.searchForm.juyuanxingming!='' && this.searchForm.juyuanxingming!=undefined){
            params['juyuanxingming'] = '%' + this.searchForm.juyuanxingming + '%'
          }
