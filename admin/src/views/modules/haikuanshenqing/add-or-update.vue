		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="贷款编号" prop="daikuanbianhao">
					<el-input v-model="ruleForm.daikuanbianhao"  placeholder="贷款编号" clearable  :readonly="ro.daikuanbianhao"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="贷款编号" prop="daikuanbianhao">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="还款金额" prop="haikuanjine">
					<el-input-number v-model="ruleForm.haikuanjine" placeholder="还款金额" :readonly="ro.haikuanjine"></el-input-number>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="还款金额" prop="haikuanjine">
					<el-input v-model="ruleForm.haikuanjine" placeholder="还款金额" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="upload" v-if="type!='info' && !ro.haikuantupian" label="还款图片" prop="haikuantupian">
					<file-upload
						tip="点击上传还款图片"
						action="file/upload"
						:limit="3"
						:multiple="true"
						:fileUrls="ruleForm.haikuantupian?ruleForm.haikuantupian:''"
						@change="haikuantupianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="upload" v-else-if="ruleForm.haikuantupian" label="还款图片" prop="haikuantupian">
					<img v-if="ruleForm.haikuantupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.haikuantupian.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.haikuantupian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="date" v-if="type!='info'" label="还款时间" prop="haikuanshijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.haikuanshijian" 
						type="datetime"
						:readonly="ro.haikuanshijian"
						placeholder="还款时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.haikuanshijian" label="还款时间" prop="haikuanshijian">
					<el-input v-model="ruleForm.haikuanshijian" placeholder="还款时间" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				haikuantupian: [
					{ required: true, message: '还款图片不能为空', trigger: 'blur' },
				],
				haikuanshijian: [
				],
				sfsh: [
	...
	created() {
		this.ruleForm.haikuanshijian = this.getCurDateTime()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='haikuanjine'){
							this.ruleForm.haikuanjine = obj[o];
							this.ro.haikuanjine = true;
							continue;
						}
						if(o=='kehuzhanghao'){
						...
						if(o=='haikuantupian'){
							this.ruleForm.haikuantupian = obj[o];
							this.ro.haikuantupian = true;
							continue;
						}
						if(o=='haikuanshijian'){
							this.ruleForm.haikuanshijian = obj[o];
							this.ro.haikuanshijian = true;
							continue;
						}
				}
	...
	if(this.ruleForm.haikuantupian!=null) {
		this.ruleForm.haikuantupian = this.ruleForm.haikuantupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `haikuanshenqing/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.haikuanshenqingCrossAddOrUpdateFlag = false;
											this.parent.search();
    ...
    haikuantupianUploadChange(fileUrls) {
	    this.ruleForm.haikuantupian = fileUrls;
    },
  }
};
</script>
